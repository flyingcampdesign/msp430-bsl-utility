#!/usr/bin/env python
#
# This is a modified version of bsl.py by Chris Liechti <cliechti@gmx.net>
# that is used in the MSP430 BSL Utility by Flying Camp Design.
# It contains a number of small changes that fix the progress_update
# functionality in program and upload.
#
# FIXES BY: Chris Wilson <cwilson@flyingcampdesign.com>
...
class BootStrapLoader(LowLevel):
    ...
    def progress_update(self, count, total):
    ...
    def programBlock(self, address, data):
        ...
        while pstart < total_length:
            ...
            #~ if self.showprogress:
                #~ self.progress_update(count, total)
    #segments:
    ...
    def programData(self, segments, action):
        ...
        if self.showprogress:
            self.progress_update(count, total)
        if DEBUG: sys.stderr.write("  Program finished.\n")
        sys.stderr.flush()
    ...
    def uploadData(self, startaddress, size, wait=0):
        ...
        if self.showprogress:
            self.progress_update(count, total)
        if DEBUG: sys.stderr.write("  Upload finished.\n")
        sys.stderr.flush()
        return data
